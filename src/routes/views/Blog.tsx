...
export default function Blog() {
  ...
  const deleteComment = async (id: string) => {
    ...
    await deleteRequest(`${localhostIP}/api/comment/${id}`, accessToken);
    navigate(0);
  }
  ...
  const handleVote = async (type: string) => {
    ...
    await putRequest<Blog>(`${localhostIP}/api/blog/vote?id=${blogData._id}&method=${type}`, accessToken || "", {})
      .then((res) => {
        setBlogData(prev => {
          return {
            ...prev,
  ...
  const commentVote = async (commentId: string, type: string) => {
    ...
    const accessToken = await getToken("access");
    await putRequest<Blog>(`${localhostIP}/api/comment/vote?id=${commentId}&method=${type}`, accessToken || "", {
      blogid: blogData._id
    })
    .then(res => {
  ...
  const handleBookmark = async () => {
    ...
    const accessToken = await getToken("access");
    await putRequest(`${localhostIP}/api/interact/bookmark?id=${blogData._id}`, accessToken || "", {})
    .then(res => {
