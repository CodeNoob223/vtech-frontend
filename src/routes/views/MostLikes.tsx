...
export default function MostLikes() {
    ...
    const getBlogs = async () => {
        try {
            const res = await axios.get(`http://${localhostIP}:3001/api/blog/mostlikes`, {});
    ...
    return (
        <div className="relative">
            <Helmet>
                <title>{user._id && (user.notifications.length > 0) ? `(${user.notifications.length})` : ""} Most likes</title>
