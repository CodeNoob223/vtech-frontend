...
function FallBackBigCard(): JSX.Element {
    return (
...
function FallBackSmallCard(): JSX.Element {
...
function CardFallBack(): JSX.Element {
    return (
                ...
                <section className="overflow-x-hidden h-[360px] category-list gap-x-5 sm:gap-x-6 animate-pulse">
                    <div className="w-max flex gap-4 scroll-right mb-5">
                        {loadingArray.map(category => {
                            return <p key={category} className="text-[50px] bg-secondary-no-gr rounded-full h-max text-transparent font-bold">{category}</p>
                        })}
                    </div>
